<?php
if(class_exists("Gcx_Mailpoet_Ex_Options_Group_Container_Renderer")){
    return;
}
class Gcx_Mailpoet_Ex_Options_Group_Container_Renderer extends Gcx_Mailpoet_Ex_Container_Renderer{
    ...
    protected function initialize_state_option(){
        $this->state_option =  Gcx_Mailpoet_Ex_CF::create_and_init("Option",array("namespace" => $this->namespace , "id" => $this->group_descriptor->get_id()."_group" , "default" => "open"));
   ...
   public function render_item($key,$description,$counter){
      if($description->has_renderer()){
        ...
        $cnt = $renderer = Gcx_Mailpoet_Ex_CF::create($renderer_name);
      ...
      switch($description->get_type()){
       ...
       public function render_type_default($description){
         $renderer = Gcx_Mailpoet_Ex_CF::create("Text_Renderer");
       $renderer->render($description);
       }

       public function render_type_number($description){
       $renderer = Gcx_Mailpoet_Ex_CF::create("Number_Renderer");
       $renderer->render($description);
       }

       public function render_type_textarea($description){
       $renderer = Gcx_Mailpoet_Ex_CF::create("Textarea_Renderer");
       $renderer->render($description);
       }

       public function render_type_checkbox($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Checkbox_Renderer");
        $renderer->render($description);
    }

    public function render_type_radio($description){
      $renderer = Gcx_Mailpoet_Ex_CF::create("Radio_Renderer");
        $renderer->render($description);
    }

       public function render_type_select($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Select_Renderer");
        $renderer->render($description);
    }

    public function render_type_color($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Color_Renderer");
        $renderer->render($description);
    }
       public function render_type_onoff($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Button_Onoff_Renderer");
        $renderer->render($description);
       }
       public function render_type_darklight($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Button_Darkligth_Renderer");
        $renderer->render($description);
       }

    public function render_type_fonttype_select($description){
        $renderer = Gcx_Mailpoet_Ex_CF::create("Fonttype_Select_Rnderer");
        $renderer->render($description);
    }

    
       public function render_type_slider($description){
           $renderer = Gcx_Mailpoet_Ex_CF::create("Slider_Renderer");
        $renderer->render($description);
       }
    ...
    public function after_item_render($key,$description,$counter){
        if($this->group_descriptor->has_sub_group($description->get_id())) {
            ...
            if($container->has_renderer()) {
                ...
                $renderer_name = Gcx_Mailpoet_Ex_CF::get_class_name("Options_Subgroup_Container_Renderer");
            }
            $repository = Gcx_Mailpoet_Ex_CF::create("Option_Repository_Factory")->get_instance();
            ...
            $cnt->set_event_prefix(GC_MAILPOET_EX_NAME);
