<?php
if(class_exists('Gcx_Mailpoet_Ex_Cache')) {
    return;
} 
class Gcx_Mailpoet_Ex_Cache 
    implements 
    Gcx_Mailpoet_Ex_Configurable_Interface
{
    ...
    public function get_cache_file_path($file_name){
        if(empty($this->cache_dir)){
            $file = Gcx_Mailpoet_Ex_Util::get_realpath().DIRECTORY_SEPARATOR."cache".DIRECTORY_SEPARATOR.$file_name;
        } else{
    ...
    public function get_cache_file_url($file_name){
        if(empty($this->cache_dir)){
            $url = Gcx_Mailpoet_Ex_Util::get_url()."/"."cache"."/".$file_name;
        } else{
