<?php
if(class_exists('Gcx_Mailpoet_Ex_Option_Group')) {
    return;
}
class Gcx_Mailpoet_Ex_Option_Group{
    protected $title = "";
    protected $id = "";
    ...
    protected function load_sub_groups_from_array($sub_groups){
        foreach($sub_groups as $key => $descriptor){
            $this->sub_groups[$key] = Gcx_Mailpoet_Ex_CF::create_and_init("Option_Group", $descriptor);
        }
    }
