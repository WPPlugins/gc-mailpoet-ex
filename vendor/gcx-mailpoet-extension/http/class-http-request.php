<?php
if(class_exists("Gcx_Mailpoet_Ex_Http_Request")){
    return;
}
interface Gcx_Mailpoet_Ex_Http_Request_Interface{
    function get($url,$parameters,$timeout = 0);

}
class Gcx_Mailpoet_Ex_Http_Request
implements Gcx_Mailpoet_Ex_Http_Request_Interface
{
    protected $timeout = 1;
    ...
    public function init_implementation(){
        if(function_exists('curl_init')) {
            $this->request_implementation = Gcx_Mailpoet_Ex_CF::create("Http_Request_Curl_Impl");
            return;
        }
        if(ini_get('allow_url_fopen')){
            $this->request_implementation = Gcx_Mailpoet_Ex_CF::create("Http_Request_Filegetcontents_Impl");
            return;
        }
        if(function_exists('http_get')){
            $this->request_implementation = Gcx_Mailpoet_Ex_CF::create("Http_Request_Httpget_Impl");
            return;
        }
...
class 
    Gcx_Mailpoet_Ex_Http_Request_Filegetcontents_Impl
implements 
    Gcx_Mailpoet_Ex_Http_Request_Interface{
    public function get($url,$parameters,$timeout = 0){
...
class 
    Gcx_Mailpoet_Ex_Http_Request_Curl_Impl
implements 
    Gcx_Mailpoet_Ex_Http_Request_Interface{
    public function get($url,$parameters,$timeout = 0){
        if(count($this->parameters)){
...
class 
    Gcx_Mailpoet_Ex_Http_Request_Httpget_Impl
implements 
    Gcx_Mailpoet_Ex_Http_Request_Interface{
    public function get($url,$parameters,$timeout = 0){
        if(count($this->parameters)){
